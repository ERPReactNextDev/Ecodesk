...
const FormFields: React.FC<FormFieldsProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "account" | "ticket" | "department" | "date"
  >("account");
  ...
  useEffect(() => {
    if (!isEditMode && !postData.TicketReferenceNumber) {
      setPostData((prev: any) => ({
        ...prev,
        TicketReferenceNumber: generateTicketReferenceNumber(),
      }));
    }
  }, [isEditMode, postData.TicketReferenceNumber, setPostData]);
    ...
    <>
      ...
      >
        ...
        <div className="hidden md:flex w-60 border-r pr-1 border-gray-300 flex-col">
          ...
          <button
            type="button"
            className={`flex items-center gap-2 px-4 py-4 rounded text-left ${activeTab === "ticket"
                ? "bg-blue-900 text-white"
                : "hover:bg-gray-100"
              }`}
          ...
          <button
            type="button"
            className={`flex items-center gap-2 px-4 py-4 rounded text-left ${activeTab === "department"
                ? "bg-blue-900 text-white"
                : "hover:bg-gray-100"
              }`}
          ...
          <button
            type="button"
            className={`flex items-center gap-2 px-4 py-4 rounded text-left ${activeTab === "date"
                ? "bg-blue-900 text-white"
                : "hover:bg-gray-100"
              }`}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="flex-1 overflow-y-auto p-4">
            ...
            <div className="md:hidden">
              ...
              {step === 2 && (
                <TicketTab postData={postData} handleChange={handleChange} />
              )}
              {step === 3 && (
                <DepartmentTab postData={postData} handleChange={handleChange} />
              )}
              {step === 4 && (
                <DateTab postData={postData} handleChange={handleChange} />
              )}
            </div>
