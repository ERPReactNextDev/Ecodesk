...
const Table: React.FC<TableProps> = ({ currentPosts, handleEdit, handleDelete }) => {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/Backend/Account/bulk-delete", {
        method: "DELETE",
      ...
      <div className="flex justify-between items-center p-4 border-b">
        ...
        <div className="flex flex-col">
          ...
          <p className="text-[10px] text-gray-600 max-w-xl leading-relaxed">
            This section provides an organized view of all customer records, allowing you to quickly access, search, and filter information. You can track customer details, interactions, and statuses efficiently to support better management and decision-making.
          </p>
          ...
          {filteredPosts.length > 0 ? (
            filteredPosts.map((post) => (
              <tr key={post._id} onClick={() => handleEdit(post)} className="cursor-pointer hover:bg-gray-100 transition-colors duration-200 whitespace-nowrap">
                ...
                <td className="px-6 py-4 text-xs text-gray-700 capitalize">{post.CityAddress || "-"}</td>
              </tr>
            ))
          ) : (
            <tr>
