    ...
    <div className="overflow-x-auto rounded-lg border border-gray-200 shadow-sm">
      <div className="flex justify-between items-center p-4 border-b">
        ...
        <div className="flex flex-col">
          <h2 className="text-lg font-semibold text-gray-800">SKU Listing's</h2>
          <p className="text-[10px] text-gray-600 max-w-xl leading-relaxed">
            This section displays a list of SKUs (Stock Keeping Units) and includes filtering options to refine the data. Users can search by keyword, adjust the number of displayed entries, and filter results within a specific date range. Additionally, an "Export to Excel" button allows users to download the SKU data for further analysis.
          </p>
        </div>
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gradient-to-r from-gray-50 to-gray-100 whitespace-nowrap">
          <tr>
            ...
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 tracking-wider">Item Category</th>
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 tracking-wider">Item Code</th>
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 tracking-wider">Item Description</th>
          ...
          {filteredPosts.length > 0 ? (
            filteredPosts.map((post) => (
              <tr key={post._id} onClick={() => handleEdit(post)} className="cursor-pointer hover:bg-gray-100 transition-colors duration-200 whitespace-nowrap">
                ...
                <td className="px-6 py-4 text-xs text-gray-700 uppercase">{post.createdAt ? new Date(post.createdAt).toLocaleString() : "-"}</td>
                <td className="px-6 py-4 text-xs font-medium text-gray-900 uppercase">{post.CompanyName}</td>
                <td className="px-6 py-4 text-xs text-gray-700 uppercase">{post.Remarks || "-"}</td>
                <td className="px-6 py-4 text-xs text-gray-700">{post.ItemCode || "-"}</td>
                <td className="px-6 py-4 text-xs text-gray-700">{post.ItemDescription || "-"}</td>
