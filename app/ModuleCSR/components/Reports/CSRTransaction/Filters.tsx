    ...
    <div className="p-4 bg-gray-50 border-b">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
        {Object.keys(filters).map((key) => {
          // For Agent and Manager, show "Firstname Lastname"
          const uniqueValues = Array.from(
            new Set(
              currentPosts
                .map((post) => {
                  if (key === "AgentLastname") return `${post.AgentFirstname} ${post.AgentLastname}`;
                  if (key === "ManagerLastname") return `${post.ManagerFirstname} ${post.ManagerLastname}`;
