...
const AgentSalesConversion: React.FC<AgentSalesConversionProps> = ({
    ...
    const getMinutesDifference = (start?: string, end?: string) => {
        ...
        // Gawin 0 lagi ang seconds at milliseconds
        startDate.setSeconds(0, 0);
        endDate.setSeconds(0, 0);

        const diffMs = endDate.getTime() - startDate.getTime();
        return diffMs / (1000 * 60); // eksaktong minutes
    };

    const formatMinutesToHM = (minutes: number) => {
        if (minutes <= 0) return "N/A";
        const h = Math.floor(minutes / 60);
        const m = Math.round(minutes % 60); // round dito na lang
        return `${h}h ${m}m`;
    };
    ...
    const calculateAgentTotals = (agentMetrics: Metric[]) => {
        return agentMetrics.reduce(
            (acc, metric) => {
                ...
                if (minutesDiff > 0) {
                    acc.totalTsaResponseTime += minutesDiff;
                    acc.tsaCount += 1;
                }
                ...
                // ✅ Non Quotation HT & Quotation HT based on remarks
                ...
                const handlingTimeMinutes = getMinutesDifference(metric.TicketReceived, metric.TsaHandlingTime);

                if (nonQuotationRemarks.includes(metric.Remarks)) {
                    acc.nonQuotationHT += handlingTimeMinutes;
                    acc.nonQuotationCount += 1;
                } else if (quotationRemarks.includes(metric.Remarks)) {
                    acc.quotationHT += handlingTimeMinutes;
            ...
            },
            {
                ...
                tsaCount: 0,
                nonQuotationHT: 0, // total minutes
                nonQuotationCount: 0, // para sa average
                quotationHT: 0,     // total minutes
                quotationCount: 0   // para sa average
            }
        );
    };
    ...
    const totals = useMemo(() => {
        return Object.values(groupedMetrics).reduce(
            (acc, agentMetrics) => {
                ...
                acc.sales += agentTotals.sales;
                acc.nonSales += agentTotals.nonSales;
                acc.totalAmount += agentTotals.totalAmount;
                acc.totalQtySold += agentTotals.totalQtySold;
                acc.totalConversionToSale += agentTotals.totalConversionToSale;
                acc.newClientCount += agentTotals.newClientCount;
                acc.newNonBuyingCount += agentTotals.newNonBuyingCount;
                acc.existingActiveCount += agentTotals.existingActiveCount;
                acc.existingInactiveCount += agentTotals.existingInactiveCount;
                acc.newClientConvertedAmount += agentTotals.newClientConvertedAmount;
                acc.newNonBuyingConvertedAmount += agentTotals.newNonBuyingConvertedAmount;
                acc.existingActiveConvertedAmount += agentTotals.existingActiveConvertedAmount;
                acc.existingInactiveConvertedAmount += agentTotals.existingInactiveConvertedAmount;

                acc.totalATU += agentTotals.totalConversionToSale > 0
                    ? agentTotals.totalQtySold / agentTotals.totalConversionToSale
                    : 0;

                acc.totalATV += agentTotals.totalConversionToSale > 0
                    ? agentTotals.totalAmount / agentTotals.totalConversionToSale
                    : 0;

                acc.totalConversionPercentage += agentTotals.sales > 0
                    ? (agentTotals.totalConversionToSale / agentTotals.sales) * 100
                    : 0;

                // TSA totals
                acc.totalTsaResponseTime += agentTotals.totalTsaResponseTime;
                acc.agentCount += 1;

                // ✅ Non Quotation & Quotation HT totals
                acc.totalNonQuotationHT += agentTotals.nonQuotationHT;
                acc.totalNonQuotationCount += agentTotals.nonQuotationCount;
                acc.totalQuotationHT += agentTotals.quotationHT;
                acc.totalQuotationCount += agentTotals.quotationCount;
            ...
            },
            {
                ...
                totalTsaResponseTime: 0,
                agentCount: 0,
                totalNonQuotationHT: 0,
                totalNonQuotationCount: 0,
                totalQuotationHT: 0,
                    ...
                    <table className="min-w-full table-auto">
                        <thead className="bg-gray-100 sticky top-0">
                            <tr className="text-xs text-left whitespace-nowrap border-l-4 border-emerald-400">
                                ...
                                <th className="px-6 py-4 font-semibold text-gray-700">Non Quotation HT</th>
                                <th className="px-6 py-4 font-semibold text-gray-700">Quotation HT</th>
                            </tr>
                        </thead>
                                ...
                                Object.keys(groupedMetrics).map((refId, index) => {
                                    ...
                                    return (
                                        <tr key={index} className="border-b whitespace-nowrap">
                                            ...
                                            <td className="px-6 py-4 text-xs">
                                                {totals.tsaCount > 0
                                                    ? formatMinutesToHM(totals.totalTsaResponseTime / totals.tsaCount) // ✅ Divide para makuha average
                                                    : "N/A"}
                                            </td>
                                            <td className="px-6 py-4 text-xs">
                                                {totals.nonQuotationCount > 0
                                                    ? formatMinutesToHM(totals.nonQuotationHT / totals.nonQuotationCount)
                                                    : "N/A"}
                                            </td>
                                            <td className="px-6 py-4 text-xs">
                                                {totals.quotationCount > 0
                                                    ? formatMinutesToHM(totals.quotationHT / totals.quotationCount)
                                                    : "N/A"}
                                            </td>
                        ...
                        <tfoot>
                            <tr className="bg-gray-200 text-xs font-bold text-left">
                                ...
                                <td className="px-6 py-4 text-xs">
                                    {totals.agentCount > 0
                                        ? formatMinutesToHM(totals.totalTsaResponseTime / totals.agentCount)
                                        : "N/A"}
                                </td>
                                <td className="px-6 py-4 text-xs">
                                    {totals.totalNonQuotationCount > 0
                                        ? formatMinutesToHM(totals.totalNonQuotationHT / totals.totalNonQuotationCount)
                                        : "N/A"}
                                </td>
                                <td className="px-6 py-4 text-xs">
                                    {totals.totalQuotationCount > 0
                                        ? formatMinutesToHM(totals.totalQuotationHT / totals.totalQuotationCount)
                                        : "N/A"}
                                </td>
